...
mod tests {
    ...
    fn register_seeds_state_and_counts_lease() {
        // Bind the monitor to an isolated temp HOME so the tick thread the
        // registration starts can never read the developer's real credential or
        // spawn a real host ping (F3).
        let home = std::env::temp_dir().join("awman-monitor-test-home");
        let monitor = CredentialRefreshMonitor::with_resolver(
            MonitorConfig::default(),
            AuthPathResolver::at_home(&home),
        );
        let dir = std::env::temp_dir().join("awman-monitor-test");
        let delivery = RefreshableCredentialDelivery {
            agent: AgentName::new("claude").unwrap(),
            spec_agent: "claude",
            credential_env_key: "CLAUDE_CODE_OAUTH_TOKEN",
