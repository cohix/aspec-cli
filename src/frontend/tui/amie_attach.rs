...
use crate::command::commands::amie::runtime_guard::AMIE_SANDBOX_REFUSAL;
...
use crate::frontend::attach::{list_condition_containers, no_run_in_progress};
...
use crate::frontend::tui::{
    AmieConditionWorkflowSource, RemoteWorkflowPoller, WorkflowStateSource,
};
...
impl AmieSlotDriver {
    ...
    fn attach_evaluation(
        ...
        match self.attach_handle("evaluation".to_string(), agent, model, handle, true) {
            Ok(()) => true,
            Err(error) => {
                self.log_error(format!("failed to attach condition evaluation: {error}"));
                false
            }
        }
    }
...
/// Start an `amie attach <name>` session in the active amie tab.
pub fn start_amie_attach(app: &mut App, condition: &str) {
    if app.engines.container_runtime.is_none() {
        app.status_bar.text =
            AMIE_SANDBOX_REFUSAL.replace("{runtime}", app.engines.runtime.runtime_name());
        return;
    }
    if !app.active_tab().is_amie {
        app.status_bar.text = "amie attach is only available from the amie tab".to_string();
        return;
    }
    ...
    let gateway = match supervisor.gateway_from_meta() {
        ...
        Ok(None) => {
            app.status_bar.text = "amie daemon not reachable".to_string();
            return;
        }
        Err(error) => {
            app.status_bar.text = error.to_string();
    ...
    let source: Arc<dyn WorkflowStateSource> = Arc::new(AmieConditionWorkflowSource::new(
        gateway,
    ...
    // the same spawn-and-result-channel pattern used to open the amie tab.
    ...
    let initial_workflow = match probe_rx.recv() {
        Ok(result) => result,
        Err(_) => {
            app.status_bar.text = "amie workflow probe was interrupted".to_string();
            return;
        }
    };
    ...
    let candidates = match list_condition_containers(app.engines.runtime.as_ref(), condition) {
    ...
    // runtime set means the condition is idle and fails immediately.
    if candidates.is_empty() && !workflow_phase {
        app.status_bar.text = no_run_in_progress(condition).to_string();
        return;
    }
    ...
    let amie_state = tab
        .amie
        .as_mut()
        .expect("is_amie tabs always carry amie state");
    ...
    tab.execution_phase = ExecutionPhase::Running {
        command: format!("amie attach {condition}"),
    };
    tab.suppress_container_auto_open = false;
    amie_state.attached_condition = Some(condition.to_string());
    amie_state
        .daemon_reachable
    ...
    let reachable = amie_state.daemon_reachable.clone();
    let cancel = amie_state.cancel.clone();
    let evaluation_metadata = amie_state
        .snapshot
        .lock()
        .ok()
        .and_then(|snapshot| {
    ...
    let driver = Arc::new(Mutex::new(
        AmieSlotDriver::new(runtime, slot_events.clone(), status_log).with_cancel(cancel.clone()),
    ));
    ...
    amie_state.set_poll_handle(poll_task);
}
