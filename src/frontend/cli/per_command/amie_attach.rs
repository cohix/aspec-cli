//! CLI attach entry point for a running amie condition.
...
use crate::frontend::attach::{
    format_candidates, label_with_step_names, list_condition_containers, resolve_attach_target,
    AttachResolution,
};
use crate::frontend::cli::{error_exit_code, format_error, CliFrontend, RuntimeContext};

/// Attach a single CLI terminal to the selected running amie container.
pub(crate) async fn run_attach(matches: &ArgMatches, ctx: &RuntimeContext) -> ExitCode {
    let Some(amie) = matches.subcommand_matches("amie") else {
        return render_error(&CommandError::unknown_command(&["amie", "attach"]));
    };
    let Some(attach) = amie.subcommand_matches("attach") else {
        return render_error(&CommandError::unknown_command(&["amie", "attach"]));
    };
    ...
    if ctx.engines.container_runtime.is_none() {
        return render_error(&CommandError::Other(
            AMIE_SANDBOX_REFUSAL.replace("{runtime}", ctx.engines.runtime.runtime_name()),
        ));
    }

    let mut candidates = match list_condition_containers(ctx.engines.runtime.as_ref(), name) {
...
async fn workflow_state_for_labels(condition: &str) -> Option<WorkflowState> {
    let supervisor = AmieSupervisor::from_env(&Env::from_process()).ok()?;
    let gateway = supervisor.gateway_from_meta().ok()??;
    let response = gateway
        .core()
        .get(&["conditions", condition, "workflow"])
        .await
        .ok()?;
