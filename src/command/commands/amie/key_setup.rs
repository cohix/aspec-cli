//! The one-time "here is your amie key" snippet.
//!
//! The amie daemon has always minted a bearer key on its first start, but the
...
//! environment variable ([`AWMAN_AMIE_KEY`]) and the shell startup file to put
...
//! and the caller decides where the text goes — stdout for the CLI, the amie
//! tab's message area for the TUI.

use crate::data::config::env::{EnvSnapshot, AWMAN_AMIE_KEY};
...
impl ShellFlavor {
    ...
    fn export_line(self, key: &str) -> String {
        match self {
            Self::Fish => format!("set -gx {AWMAN_AMIE_KEY} {key}"),
            _ => format!("export {AWMAN_AMIE_KEY}={key}"),
        }
    }
}
...
pub fn render_key_setup(key: &str, shell: ShellFlavor) -> String {
    ...
    out.push_str("\n\n");
    out.push_str(&format!(
        "Add this to {} so the awman CLI and TUI can authenticate to amie:\n\n    {}\n\n",
        shell.rc_file(),
        shell.export_line(key)
    ));
    out.push_str(
        "Until you do, export it in the current shell — `awman amie` commands\n\
    ...
    out.push_str(
        "Prefer to run without a key? Stop the daemon and start it with\n    \
         ...
         which mints no key and accepts unauthenticated requests. amie binds to\n\
...
fn render_banner(key: &str) -> String {
    let title = "amie API key (store this — it will not be shown again)";
...
mod tests {
    ...
    fn snippet_exports_the_documented_env_var_with_the_key() {
        ...
        assert!(
            out.contains("export AWMAN_AMIE_KEY=deadbeef"),
    ...
    fn fish_gets_set_gx_rather_than_export() {
        ...
        assert!(
            out.contains("set -gx AWMAN_AMIE_KEY deadbeef"),
            "fish has no `export`; got:\n{out}"
        );
        assert!(!out.contains("export AWMAN_AMIE_KEY"), "got:\n{out}");
