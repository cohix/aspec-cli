//! WI 0101 — E2E: the real `curl` executable drives the full condition
//! lifecycle against a running amie daemon with **no `awman` frontend
...
//! `RemoteConditionGateway`, the CLI) is on the path — only the daemon's
//! loopback socket is.
//!
//! Boots the real daemon via `frontend::amie::serve_with` (the same
//! injectable seam `tests/amie_daemon_http.rs` uses) and drives it purely
...
async fn start_daemon(root: &std::path::Path) -> (tokio::task::JoinHandle<()>, String) {
    ...
    let handle = tokio::spawn(async move {
        let _ =
            awman::frontend::amie::serve_with(config, engines, Arc::new(NeverTriggeredEvaluator))
                .await;
    });
...
async fn curl_drives_the_full_condition_lifecycle_with_no_awman_frontend() {
    ...
    // status — daemon is up, no conditions yet.
    let (code, status) = curl_get(format!("{base}/v1/status")).await;
    assert_eq!(code, 200);
    assert_eq!(status["running"], true);
    assert_eq!(status["condition_count"], 0);
    ...
    .await;
    assert_eq!(code, 200, "amie add must succeed: {created}");
    assert_eq!(created["name"], "issue-triage");
    assert_eq!(created["status"], "active");

    // list
    let (code, list) = curl_command(&base, "amie list", &[]).await;
    assert_eq!(code, 200);
    assert_eq!(list.as_array().unwrap().len(), 1);

    // show — one response shape carrying the condition *and* its run history,
    // which is what both gateway implementations deserialize.
    let (code, shown) = curl_command(&base, "amie show", &["issue-triage"]).await;
    assert_eq!(code, 200);
    assert_eq!(shown["condition"]["name"], "issue-triage");
    assert_eq!(
        shown["runs"].as_array().map(Vec::len),
        Some(0),
        "a never-evaluated condition has an empty (not absent) run history: {shown}"
    );

    // pause
    let (code, _) = curl_command(&base, "amie pause", &["issue-triage"]).await;
    assert_eq!(code, 200);
    let (_, shown) = curl_command(&base, "amie show", &["issue-triage"]).await;
    assert_eq!(shown["condition"]["status"], "paused");

    // resume
    let (code, _) = curl_command(&base, "amie resume", &["issue-triage"]).await;
    assert_eq!(code, 200);
    let (_, shown) = curl_command(&base, "amie show", &["issue-triage"]).await;
    assert_eq!(shown["condition"]["status"], "active");

    // workflow — 404 until a running evaluation persists a workflow state path.
    let (code, _) = curl_get(format!("{base}/v1/conditions/issue-triage/workflow")).await;
    assert_eq!(code, 404);

    // an unknown condition is a 404, not a 500 or an empty 200.
    let (code, _) = curl_get(format!("{base}/v1/conditions/no-such-condition/workflow")).await;
    assert_eq!(code, 404);

    // status reflects the one active condition.
    let (_, status) = curl_get(format!("{base}/v1/status")).await;
    assert_eq!(status["condition_count"], 1);
    assert_eq!(status["active_count"], 1);

    // remove
    let (code, _) = curl_command(&base, "amie remove", &["issue-triage"]).await;
    assert_eq!(code, 200);
    let (_, list) = curl_command(&base, "amie list", &[]).await;
...
/// holding real session and command rows) starts the amie daemon, gains the
/// amie tables, and retains every prior API-mode row.
///
...
/// database is the amie daemon. Nothing may be lost, and the legacy original
/// must survive under its `.pre-migration` name for `awman clean` to remove.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn a_pre_migration_install_starts_amie_gains_its_tables_and_keeps_api_rows() {
    ...
    // ── upgrade: the amie daemon is the first process to open the database ──
    let (handle, base) = start_daemon(tmp.path()).await;
    ...
    // ── and the same file now carries amie's tables, driven over HTTP ──
    let (code, created) = curl_command(
        &base,
        "amie add",
        &[
    ...
    assert_eq!(
        code, 200,
        "amie add must succeed on a migrated database: {created}"
    );
    let (_, list) = curl_command(&base, "amie list", &[]).await;
    assert_eq!(list.as_array().unwrap().len(), 1);

    // The amie rows landed in the shared database, not a second file.
    let conn = rusqlite::Connection::open(&data_db).unwrap();
    let conditions: i64 = conn
        .query_row("SELECT COUNT(*) FROM amie_conditions", [], |row| row.get(0))
        .expect("the shared database must carry amie_conditions after startup");
