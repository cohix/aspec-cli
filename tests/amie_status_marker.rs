//! WI 0101 — `awman status` renders the `amie:<condition>` source marker.
//!
...
//! `list_running_all()` returns one amie-launched container and one plain
...
//!
//! Docker- and Apple-gated tiers then plant a real amie-named container and
...
#[tokio::test]
async fn status_marks_amie_container_and_leaves_session_container_unmarked() {
    let amie_name = generate_amie_container_name("issue-triage");
    let runtime = Arc::new(FakeRuntime {
        handles: vec![handle(&amie_name), handle("awman-1234-5678")],
    });

    let outcome = run_status(runtime).await;
    assert_eq!(outcome.containers.len(), 2);

    let amie_row = outcome
        .containers
        .iter()
        .find(|c| c.name == amie_name)
        .expect("amie container row must be present");
    assert_eq!(
        amie_row.source,
        ContainerSource::Amie("issue-triage".to_string())
    );
    assert_eq!(
        amie_row.source_label().as_deref(),
        Some("amie:issue-triage")
    );
    assert_eq!(amie_row.kind, ContainerKind::Agent);
...
#[tokio::test]
async fn status_renders_amie_marker_in_the_dashboard_text() {
    ...
    let amie_name = generate_amie_container_name("nightly-audit");
    let runtime = Arc::new(FakeRuntime {
        handles: vec![handle(&amie_name)],
    });
    let engines = engines_with(runtime);
    let cmd = StatusCommand::new(StatusCommandFlags { watch: false }, engines);
    ...
    assert!(
        combined.contains("amie:nightly-audit"),
        "dashboard text must contain the amie marker: {combined}"
    );
}
...
// `list_running_all()`: each tier plants one amie-named container and one
...
/// Plant one amie container and one session container on a live backend and
/// assert `awman status` marks exactly the amie one.
async fn assert_status_marks_amie_on(cli: &str, runtime: Arc<ContainerRuntime>) {
    let suffix = unique_suffix();
    let slug = format!("audit-{suffix}");
    let amie_name = generate_amie_container_name(&slug);
    let session_name = format!("awman-{suffix}-0");

    for name in [&amie_name, &session_name] {
        if !cli_ok(
            cli,
            &["run", "-d", "--name", name, "alpine:latest", "sleep", "120"],
        ) {
            remove_container(cli, &amie_name);
    ...
    let amie_row = outcome
        ...
        .iter()
        .find(|c| c.name == amie_name)
        .unwrap_or_else(|| panic!("`awman status` must list the amie container {amie_name}"));
    assert_eq!(
        amie_row.source,
        ContainerSource::Amie(slug.clone()),
        "an amie-launched container must render as amie:{slug}"
    );
    assert_eq!(
        amie_row.source_label().as_deref(),
        Some(&*format!("amie:{slug}"))
    );
...
#[tokio::test]
async fn docker_status_marks_amie_containers_when_backend_is_available() {
    ...
    assert_status_marks_amie_on("docker", runtime).await;
}

// ─── Apple tier (skip cleanly if unavailable) ───────────────────────────────

#[tokio::test]
async fn apple_status_marks_amie_containers_when_backend_is_available() {
    ...
    assert_status_marks_amie_on("container", runtime).await;
}
