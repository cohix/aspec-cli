...
- **Every run is autonomous and PTY-backed.** There's no human around to
  ...
  auto-advance guardrails as `--yolo` — see [Yolo Mode](06-yolo-mode.md).
...
| `guidance` | Standing instructions applied to **every** task evaluation and every workflow it generates — the same mechanism as [dynamic workflow guidance](13-dynamic-workflows.md#guidance), injected as a bulleted "Developer Guidance" block in the agent's prompt. |
...
flags like `--port`. See [API Mode: API server and squad daemon](09-api-mode.md#api-server-and-squad-daemon)
